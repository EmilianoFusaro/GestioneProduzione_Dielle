...
def genera_laccato(carico,nomestampa)
     ...
     DB_ECAD.fetch(sql) do |rigax|
      ...
      if conta==0
       ...
       if (appo_cod !=rigax[:pcod]) or (rigax[:pv2] != appo_dividi)
          tot_cod=conta_cod.to_s
          conta_cod=rigax[:qta]
          appo_cod=rigax[:pcod]
          eval(TraduciComandi('LINEAX (1,24,11,24)',sposta_record))
          #eval(TraduciComandi('RETTANGOLO (1,9,10,1)',sposta_record-8))
       ...
       if ((rigax[:pv2] != appo_dividi) and (sposta_record>0)) or (sposta_record>230)
           pdf.start_new_page
           testa.split("\n").each{ |rigatesta|   
     ...
     pdf.render_file "K:/Ecadpro/RubyLogyUnion/LACCATI/#{carico}/#{nomestampa.gsub(".mst","")}2_copia.pdf"
...
genera_laccato(carico,"Stampa_Laccati_X_Variante.mst")
