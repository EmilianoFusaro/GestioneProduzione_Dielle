...
if carico.include? "XXXX"
  modulo_originale1=CaricaModuloStampa("K:/Ecadpro/RubyLogyUnion/ModuliStampa/slip_mancantiN_test.mst",0,0)
  modulo_originale2=CaricaModuloStampa("K:/Ecadpro/RubyLogyUnion/ModuliStampa/slip_mancantiN_test.mst",427,0)
  modulo_originale3=CaricaModuloStampa("K:/Ecadpro/RubyLogyUnion/ModuliStampa/slip_mancantiN_test.mst",0,-280)
  modulo_originale4=CaricaModuloStampa("K:/Ecadpro/RubyLogyUnion/ModuliStampa/slip_mancantiN_test.mst",427,-280)
else
  modulo_originale1=CaricaModuloStampa("K:/Ecadpro/RubyLogyUnion/ModuliStampa/slip_mancantiN.mst",0,0)
  modulo_originale2=CaricaModuloStampa("K:/Ecadpro/RubyLogyUnion/ModuliStampa/slip_mancantiN.mst",427,0)
  modulo_originale3=CaricaModuloStampa("K:/Ecadpro/RubyLogyUnion/ModuliStampa/slip_mancantiN.mst",0,-280)
  modulo_originale4=CaricaModuloStampa("K:/Ecadpro/RubyLogyUnion/ModuliStampa/slip_mancantiN.mst",427,-280)
end
...
#sql += " WHERE (TORDINE.CARICO = '#{carico}' #{$filtroordini}) AND (EcadTempSlip.PCOD IS NULL) AND (RORDINE.TIPORIGA = '3')  Order By RORDINE.NUMERO,RORDINE.RIGA"
#LA RIGA SOTTO CONSIDERA SOLO PADRI CON VARIANTI LA RIGA SOPRA SOLO PADRI (EVENTUALMENTE SI PUO' SOSTITUIRE PIU' IN LA )
sql += " WHERE (TORDINE.CARICO = '#{carico}' #{$filtroordini}) AND (EcadTempSlip.PCOD IS NULL) AND (RORDINE.TIPORIGA = '3') AND (RORDINE.[VAR] LIKE '%=%') Order By RORDINE.NUMERO,RORDINE.RIGA"
...
DB_3CAD.fetch(sql) do |riga|
  ...
  #NON CONTROLLANDO LA PRESENZA HO COMMENTATO IL CICLO INTERNO
...
pdf.render_file "K:/Ecadpro/RubyLogyUnion/CARICHI/#{carico}/#{carico}_SlipCicliMancantiNormali.pdf"
